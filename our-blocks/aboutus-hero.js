...
function EditComponent(props) {
  ...
  function handleProfileTitleChange(x) {
    props.setAttributes({ profileTitle: x })
  }

  function handleProfileTextChange(x) {
    props.setAttributes({ profileText: x })
  }
  ...
  const renderParagraphs = (text) => {
    // Replace newlines with opening and closing <p> tags
    const paragraphs = text.replace(/\n/g, '</p><p>');

    // Wrap the result with <p> tags
